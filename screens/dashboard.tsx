...
const DashboardScreen = ({ route }: { route: any }) => {
    ...
    const fetchDataApi = async (type: any) => {
        ...
        let setURL;
        if (type == i18n.t('Bottom-Navigation.Receiving')) {
            setURL = "GetGoodsReceiving";
        } else if (type == i18n.t('Bottom-Navigation.Outgoing')) {
            setURL = "GetGoodsIssue";
        } else if (type == i18n.t('Bottom-Navigation.Overall')) {
            setURL = "GetOverall";
        }
            ...
            if (response.json().isSuccess == true) {
                ...
                if (response.json().customerData.length != 0) {
                    setFetchedData(response.json().customerData.map((item: {
                        ...
                        amount: type == i18n.t('Bottom-Navigation.Overall') ? item.overallAmount : item.handlingChargesAmount,
                ...
                setBarData2(response.json().barChart.map(type == i18n.t('Bottom-Navigation.Overall') ? (item: { overallAmount: number; days: any; date: any }) => ({
                ...
                const WeightArray = (response.json().barChart.map(type == i18n.t('Bottom-Navigation.Overall') ? (item: { overallAmount: any; }) => item.overallAmount : (item: { handlingChargesAmount: any; }) => item.handlingChargesAmount));
    ...
    const FlatListItem = ({ item }: { item: showData }) => {
        ...
        return (
            <TouchableOpacity onPress={async () => {
                ...
                if (route.params.stayPage == i18n.t('Bottom-Navigation.Overall')) {
                ...
                <View style={css.listItem} key={parseInt(item.key)}>
                    <View style={[css.cardBody]}>
                        ...
                        {route.params.stayPage == i18n.t('Bottom-Navigation.Overall') ? (
                            <View style={{ alignItems: 'flex-start', justifyContent: 'center', flex: 1, flexGrow: 1, }}>
                                ...
                                <View style={{ flexDirection: 'row', }}>
                                    <Text style={css.text2ndHeader}>
                                        {i18n.t('Dashboard-Overall.Pallet')}: {item.palletBalance}
                                    </Text>
                                    <Text style={[css.textDescription, { textAlign: "right" }]}></Text>
                                </View>
                                <View style={{ flexDirection: 'row', }}>
                                    <Text style={css.text2ndHeader}>
                                        {i18n.t('Dashboard-Overall.Carton')}: {item.cartonBalance}
                                    </Text>
                                    <Text style={[css.textDescription, { textAlign: "right" }]}></Text>
                                </View>
                            </View>
                            ...
                            {route.params.stayPage == "Overall" ? (
                                <View style={{ width: "75%" }}>
                                    <Text style={{ fontSize: 20, fontWeight: 'bold', textAlign: "center", fontStyle: "italic" }}>
                                        {route.params.stayPage}  {i18n.t('Dashboard-Overall.Amount')}: {setNumberFormat2(totalAmount)}
                                    </Text>
                                </View>
                            ) : (
                                ...
                                <View style={{ width: "25%", alignItems: 'flex-start', justifyContent: 'center', margin: 5 }}>
                                    <View style={css.row}>
                                        <Text style={{ fontSize: 8, fontWeight: 'bold', textAlign: "left", fontStyle: "italic" }}>
                                            RA: {i18n.t('Dashboard-Overall.Rental-Amount')}
                                        </Text>
                                    </View>
                                    <View style={css.row}>
                                        <Text style={{ fontSize: 8, fontWeight: 'bold', textAlign: "left", fontStyle: "italic" }}>
                                            GR: {i18n.t('Dashboard-Overall.Good-Receive')}
                                        </Text>
                                    </View>
                                    <View style={css.row}>
                                        <Text style={{ fontSize: 8, fontWeight: 'bold', textAlign: "left", fontStyle: "italic" }}>
                                            GI: {i18n.t('Dashboard-Overall.Good-Issue')}
                                        </Text>
                                    </View>
                                </View>
                            ) : (
